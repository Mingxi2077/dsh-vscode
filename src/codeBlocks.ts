...
const FILE_RE =
  /[A-Za-z0-9_][A-Za-z0-9_.\/\\-]*\.(?:tsx?|jsx?|py|rs|go|java|c(?:pp|xx)?|h(?:pp|xx)?|json|md|ya?ml|css|html?|vue|svelte|sh|ps1|sql|toml|ini|txt|rb|php|cs|kt|swift|dart|scss|less|xml|env)\b/;
...
/** 某行之前最近的一个非空、非围栏行。 */
function precedingLine(lines: string[], beforeIndex: number): string {
  for (let j = beforeIndex; j >= 0; j--) {
    const line = lines[j];
    if (line === undefined) break;
    if (line.trim() && !/^```/.test(line)) return line.trim();
  }
  return "";
}
